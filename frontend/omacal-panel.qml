...
Panel {
    ...
    readonly property var flattenedDays: (function() {
        var days = computeRangeDays()
        var flat = []
        for (var r = 0; r < days.length; r++) {
            for (var c = 0; c < days[r].length; c++) {
                flat.push(days[r][c])
            }
        }
        return flat
    ...
    property string rangeEndLabelStr: ""
    ...
    function initRange() {
        ...
        var endJs = new Date(viewYear, viewMonth + 1, 0)
        rangeEndLabelStr = ""
    }
